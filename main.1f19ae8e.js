...
},{"_css_loader":"../../../../usr/local/lib/node_modules/parcel/src/builtins/css-loader.js"}],"brain.png":[function(require,module,exports) {
module.exports = "/brain.dffd15b5.png";
...
var getAngle = function getAngle(anchor, point) {
...
var nodePoint = function nodePoint(x, y, scale) {
  return [y * Math.cos(x -= Math.PI / 2) * scale, y * Math.sin(x) * scale];
}; //   get the distance between two points


var getHypot = function getHypot(p1, p2) {
...
var biasTitlesPos = [{
  x: width - width / 4 + biasTitleWidth,
  y: height / 4 - 10
}, {
  x: width - width / 4 + biasTitleWidth + 30,
  y: height - height / 4 + 15
}, {
  x: width / 4 - biasTitleWidth - 30,
  y: height - height / 4
}, {
  x: width / 4 - biasTitleWidth - 50,
  y: height / 4 - 30
}]; // const footerText = ``;
// initialize layout items
// initialize bias tree object

var tree = d3.tree().size([2 * Math.PI, radius / scale * 1.6]).separation(function (a, b) {
  return (a.parent == b.parent ? 1 : 2) / a.depth;
}); // initialize svg object

var svg = d3.select('.main').append('svg').attr('width', width + margin.right + margin.left).attr('height', height + margin.top + margin.bottom); // insert header

var header = d3.select('.main').append('h1').text('Cognitive Bias Codex'); // place brain image

var brainImage = d3.select('.main').append('img').attr('src', _brain.default).attr('class', 'brain-image').attr('style', function () {
  var width = 50 * scale * (radius / (radius * 0.8));
  var height = 40 * scale * (radius / (radius * 0.8));
  var left = center.x - width / 2;
  var top = center.y - height / 2;
  return "left: ".concat(left, "px; top: ").concat(top, "px; width: ").concat(width, "px; height: ").concat(height, "px");
});
/*const footer = d3
    .select('.main')
    .append('div')
...
var g = svg.append('g').attr('transform', "translate(".concat(center.x, ",").concat(center.y, ")")); // load the external data
...
function update(source) {
  ...
  var nodes = d3.hierarchy(root, function (_ref) {
    var children = _ref.children;
    return children;
  });
  nodes = tree(nodes); // define the tree links

  var link = g.selectAll('.link').data(nodes.links()).enter().append('path').attr('class', function (_ref2) {
    var target = _ref2.target;
    return "link link-".concat(target.data.id);
  }).attr('d', d3.linkRadial().angle(function (_ref3) {
    var x = _ref3.x;
    return x;
  }).radius(function (_ref4) {
    var y = _ref4.y;
    return y;
  }));
  g.append('circle').attr('r', function () {
    var nodes2Tier = nodes.descendants().filter(function (_ref5) {
    ...
    return rad;
  }).style('fill', 'none').attr('class', 'main-circle'); //   create node groups

  var node = g.selectAll('.node').data(nodes.descendants()).enter().append('g').attr('class', function (d) {
    return "node node-".concat(d.data.id);
    ...
    if (depth == 3) {
      return "translate(".concat(radialPoint(x, y), ")");
    }
  });
  node.append('circle').attr('r', function (_ref7) {
  ...
  }).attr('cx', function (_ref8) {
    var depth = _ref8.depth,
        x = _ref8.x,
        y = _ref8.y;

    if (depth == 2) {
      var point = nodePoint(x, y, scale);
      return point[0];
    }
  }).attr('cy', function (_ref9) {
  ...
  node.append('text').attr('dy', '0.31em').attr('x', function (_ref10) {
    var x = _ref10.x,
        children = _ref10.children;
    return x < Math.PI === !children ? 6 : -6;
  }).attr('text-anchor', function (_ref11) {
    var x = _ref11.x,
        children = _ref11.children;
    return x < Math.PI === !children ? 'start' : 'end';
  }).attr('transform', function (_ref12) {
    var x = _ref12.x;
    return "rotate(".concat((x < Math.PI ? x - Math.PI / 2 : x + Math.PI / 2) * 180 / Math.PI, ")");
  }).text(function (d) {
    if (d.depth == 3) {
      return d.data.name;
    }
    ...
    return "".concat(size.toFixed(2), "em");
  });
  d3.select('.biases').attr('style', function () {
    return "left: ".concat(center.x, "px; top: ").concat(center.y, "px;");
  }); // insert bias text

  var bias = d3.select('.biases').selectAll('.bias').data(function () {
    var nodesBiases = nodes.descendants().filter(function (_ref13) {
      var depth = _ref13.depth;
      return depth == 2;
    });
    return nodesBiases;
  }).enter().append('div').attr('class', 'bias').attr('style', function (d) {
    ...
    return "left: ".concat(left, "px; top: ").concat(top, "px; text-align: ").concat(align, "; width: ").concat(biasWidth, "px;");
  }).append('span').attr('class', function (d) {
    return "text text-".concat(d.data.id);
  }).text(function (d) {
    return d.data.name;
  }); //   insert bias titles

  var biasTitle = d3.select('.main').selectAll('.bias-title').data(function () {
    var nodeTitles = nodes.descendants().filter(function (_ref14) {
      var depth = _ref14.depth;
      return depth == 1;
    });
    return nodeTitles;
  }).enter().append('div').attr('class', function (d) {
    return "bias-title node-".concat(d.data.id);
    ...
    return "left: ".concat(biasTitlesPos[ix].x, "px; top: ").concat(biasTitlesPos[ix].y, "px; width: ").concat(biasTitleWidth, "px;");
  }).text(function (d) {
    return d.data.name;
  });
}
},{"./style.css":"style.css","./brain.png":"brain.png","./data.json":"data.json","d3":"node_modules/d3/index.js"}],"../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
